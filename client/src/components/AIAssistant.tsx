    ...
    <div className="h-full flex flex-col bg-[var(--background)]">
      ...
      <div className="flex-none p-3 border-b border-[var(--border)] bg-gradient-to-r from-[var(--muted)] to-[var(--secondary)]">
        <div className="flex items-center space-x-2 mb-1">
          <div className={`w-2 h-2 rounded-full ${isProcessing ? 'bg-[var(--yellow)] animate-pulse' : 'bg-[var(--green)]'}`}></div>
          <span className="text-sm font-bold text-[var(--foreground)]">AI Assistant</span>
        </div>
        <p className="text-xs text-[var(--muted-foreground)]">Smart audio analysis</p>
      </div>
      ...
      <div className="flex-1 p-3 space-y-3 overflow-y-auto scrollbar-thin">
        ...
        {suggestions.length > 0 && (
          <div className="bg-gradient-to-r from-[var(--muted)] to-[var(--secondary)] p-3 rounded-lg border border-[var(--border)]">
            <div className="text-xs font-semibold text-[var(--primary)] mb-2">AI Suggestion:</div>
            <div className="text-xs text-[var(--foreground)] mb-3 leading-relaxed">"{suggestions[currentSuggestion]}"</div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <Button variant="outline" size="sm" className="text-xs h-10 flex flex-col items-center justify-center border-[var(--border)] hover:bg-[var(--accent)]">
