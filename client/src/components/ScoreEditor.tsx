...
export function ScoreEditor({
  ...
  const [currentTool, setCurrentTool] = useState<
    'select' | 'note' | 'rest' | 'chord' | 'dynamics' | 'text'
  >('select');
  ...
  const getNotePosition = useCallback(
    (pitch: number, clef: string, staffY: number, staffLineSpacing: number) => {
      let noteY = staffY;
      ...
      if (noteY < staffTop) {
        for (
          let y = staffTop - staffLineSpacing;
          y >= noteY;
          y -= staffLineSpacing
        ) {
        ...
        for (
          let y = staffBottom + staffLineSpacing;
          y <= noteY;
          y += staffLineSpacing
        ) {
          needsLedgerLines.push(y);
  ...
  const renderScore = useCallback(() => {
    ...
    staffs.forEach((staff, staffIndex) => {
      ...
      const timeSignatureX =
        keySignatureX + keySignatureAccidentals.length * 12 + 20;
      ctx.fillText(
        staff.timeSignature[0].toString(),
        timeSignatureX,
        staffY + staffLineSpacing
      );
      ctx.fillText(
        staff.timeSignature[1].toString(),
        timeSignatureX,
        staffY + staffLineSpacing * 3
      );
      ...
      staff.notes.forEach((note) => {
        const measureNumber = Math.floor(
          note.startTime / staff.timeSignature[0]
        );
        const beatInMeasure = note.startTime % staff.timeSignature[0];
        const noteX =
          firstMeasureX +
          measureNumber * measureWidth +
          (beatInMeasure / staff.timeSignature[0]) * measureWidth;

        const { noteY, needsLedgerLines } = getNotePosition(
          note.pitch,
          staff.clef,
          staffY,
          staffLineSpacing
        ...
        if (note.duration < 4) {
          const stemDirection =
            noteY < staffY + staffLineSpacing * 2.5 ? 1 : -1;
  ...
  const handleCanvasClick = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      const beatPosition =
        Math.round((positionInMeasure * staff.timeSignature[0]) / subdivision) *
        subdivision;
      ...
      if (currentTool === 'note') {
        ...
        const { pitch, needsAccidental } = calculatePitchFromPosition(
          y,
          staffY,
          staff.clef,
          staff.keySignature
        ...
        const existingNoteIndex = staff.notes.findIndex(
          (note: Note) =>
            Math.abs(note.startTime - startTime) < 0.05 &&
          ...
          setStaffs((prev) =>
            prev.map((s) =>
              s.id === staff.id
                ? {
                    ...s,
                    notes: s.notes.filter((_, i) => i !== existingNoteIndex),
                  }
                : s
            )
          );
        ...
        const { pitch } = calculatePitchFromPosition(
          y,
          staffY,
          staff.clef,
          staff.keySignature
  ...
  // Calculate pitch from staff position with proper music theory
  const calculatePitchFromPosition = useCallback(
    (y: number, staffY: number, clef: string, keySignature: string) => {
      const lineSpacing = 8;
  ...
  const findNoteAtPosition = useCallback(
    (staff: any, time: number, pitch: number) => {
      return staff.notes.find(
        (note: Note) =>
          Math.abs(note.startTime - time) < 0.1 &&
          Math.abs(note.pitch - pitch) < 1
      );
    },
    []
  );
      ...
      switch (e.key.toLowerCase()) {
        ...
        case 'a':
          if (e.ctrlKey) {
            ...
            if (currentStaff) {
              setSelectedNotes(
                new Set(currentStaff.notes.map((note) => note.id))
              );
            }
          }
          break;
        ...
        switch (e.key) {
          case '3': // Shift + 3 = #
            setCurrentAccidental(
              currentAccidental === 'sharp' ? null : 'sharp'
            );
            break;
          ...
          notes: staff.notes.map((note) => {
            ...
            switch (direction) {
              ...
              case 'left':
                return {
                  ...note,
                  startTime: Math.max(0, note.startTime - amount * 0.25),
                };
              case 'right':
  ...
  const pasteNotes = useCallback(
    (targetTime: number, targetStaffId: string) => {
      if (copiedNotes.length === 0) return;

      const earliestTime = Math.min(
        ...copiedNotes.map((note) => note.startTime)
      );
      ...
      setStaffs((prev) =>
        prev.map((staff) =>
          staff.id === targetStaffId
            ? {
                ...staff,
                notes: [...staff.notes, ...newNotes].sort(
                  (a, b) => a.startTime - b.startTime
                ),
              }
            : staff
    ...
    Notation: [
      {
        ...
        icon: '𝅝',
        label: 'Whole Note',
        action: () => setNoteValue(4),
      },
      {
        id: 'half',
        icon: '𝅗𝅥',
        label: 'Half Note',
        action: () => setNoteValue(2),
      },
      {
        id: 'quarter',
        icon: '𝅘𝅥',
        label: 'Quarter Note',
        action: () => setNoteValue(1),
      },
      {
        id: 'eighth',
        icon: '𝅘𝅥𝅮',
        label: 'Eighth Note',
        action: () => setNoteValue(0.5),
      },
      {
        id: 'sixteenth',
        icon: '𝅘𝅥𝅯',
        label: 'Sixteenth Note',
        action: () => setNoteValue(0.25),
      },
    ],
    'Accidentals & Pitch': [
      {
        ...
        icon: '♯',
        label: 'Sharp',
        action: () => setCurrentAccidental('sharp'),
      },
      {
        id: 'flat',
        icon: '♭',
        label: 'Flat',
        action: () => setCurrentAccidental('flat'),
      },
      {
        id: 'natural',
        icon: '♮',
        label: 'Natural',
        action: () => setCurrentAccidental('natural'),
      },
      {
        id: 'clear',
        icon: '✕',
        label: 'Clear',
        action: () => setCurrentAccidental(null),
      },
    ],
    'Clefs & Signatures': [
    ...
    'Dynamics & Expression': [
      {
        ...
        icon: 'pp',
        label: 'Piano',
        action: () => setCurrentDynamic('pp'),
      },
      {
        id: 'mf',
        icon: 'mf',
        label: 'Mezzo Forte',
        action: () => setCurrentDynamic('mf'),
      },
      {
        id: 'ff',
        icon: 'ff',
        label: 'Fortissimo',
        action: () => setCurrentDynamic('ff'),
      },
    ],
    'Structure & Layout': [
      ...
      <div className="w-64 bg-[var(--muted)] border-r border-[var(--border)] flex flex-col">
        <div className="border-b border-[var(--border)]">
          <Select
            ...
            onValueChange={setSelectedPaletteCategory}
          >
        ...
        <div className="flex-1 p-2 overflow-y-auto">
          <div className="grid grid-cols-3 gap-1">
            {paletteCategories[
              selectedPaletteCategory as keyof typeof paletteCategories
            ]?.map((item) => (
              <Button
                key={item.id}
                variant="ghost"
      ...
      <div className="flex-1 flex flex-col">
        <div className="h-12 bg-[var(--muted)] border-b border-[var(--border)] px-4 flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <Select
              value={viewMode}
              onValueChange={(value: 'page' | 'continuous' | 'single') =>
                setViewMode(value)
              }
            >
        ...
        <div className="p-4 border-b border-[var(--border)]">
          ...
          <div className="mb-3">
            ...
            <Select
              ...
              onValueChange={setCurrentKeySignature}
            >
          ...
          {staffs.map((staff, index) => (
            ...
            >
              ...
              <div className="text-xs text-[var(--muted-foreground)] mb-2">
                {staff.clef === 'treble' ? '𝄞' : '𝄢'} {staff.keySignature}{' '}
