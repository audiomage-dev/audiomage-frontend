...
export function CompactTimelineEditor({ tracks, transport, zoomLevel: externalZoomLevel = 1, bpm = 120, timeSignature = [4, 4], snapMode = 'grid', gridDisplayMode = 'timecode', onTrackMute, onTrackSolo, onTrackSelect, onClipMove, onClipResize, onZoomChange, isLocked = false, collapsedGroups = new Set(), onToggleGroupCollapse }: CompactTimelineEditorProps) {
  ...
  const [rightClickedOnRange, setRightClickedOnRange] = useState(false);
  ...
  const handleContextualAction = useCallback((action: string, context: any) => {
    ...
    switch (action) {
      ...
      case 'copy':
      case 'delete':
        console.log(`${action} clip:`, context.clipId);
        break;
      case 'mute-track':
  ...
  const handleTrackRightClick = useCallback((e: React.MouseEvent, trackId: string) => {
    ...
    const context = {
      trackId,
      trackName: tracks.find(t => t.id === trackId)?.name || 'Unknown Track',
      selectedCount: targetTracks.length
    };
    
    const suggestions = generateContextualSuggestions('track-header', context);
  ...
  const handleMultiSelectionRightClick = useCallback((e: React.MouseEvent, trackId: string) => {
    ...
    if (multiSelection && !multiSelection.isActive) {
      ...
      const context = {
        ...
        selectedCount: multiSelection.selectedClips?.length || 0,
        rangeData: { startTime: multiSelection.startTime, endTime: multiSelection.endTime }
      };
      
      const suggestions = generateContextualSuggestions('multi-selection', context);
  ...
  return (
    <div className="compact-timeline-editor flex h-full bg-[var(--background)]">
        ...
        >
          ...
          >
            ...
            {rangeSelection && rangeSelection.isActive && rangeSelection.startTime !== rangeSelection.endTime && (
              ...
              >
                {/* Time display at top */}
                <div className="absolute -top-6 left-0 text-xs text-blue-600 font-mono bg-white dark:bg-gray-800 px-1 rounded pointer-events-none">
                  {rangeSelection.startTime.toFixed(2)}s
                </div>
                <div className="absolute -top-6 right-0 text-xs text-blue-600 font-mono bg-white dark:bg-gray-800 px-1 rounded pointer-events-none">
      ...
      {/* Old context menu system removed - now using IntelligentContextMenu component */}
    </div>
  );
}