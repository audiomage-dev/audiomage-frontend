...
export function ChannelStrip({ channel, onVolumeChange, onMuteToggle, onSoloToggle }: ChannelStripProps) {
  ...
  const getChannelColor = () => {
    ...
    if (channel.name.includes('Drums')) return 'text-[hsl(var(--aurora-orange))]';
    if (channel.name.includes('Bass')) return 'text-[hsl(var(--aurora-yellow))]';
  ...
  return (
    <div className={`bg-[hsl(var(--nord-2))] rounded p-2 ${channel.name.includes('AI') ? 'ai-glow' : ''}`}>
        ...
        <div className="flex space-x-1">
          <Button
            ...
            className={`w-4 h-4 rounded-sm text-xs h-auto p-0 ${
              !channel.muted ? 'bg-[hsl(var(--aurora-green))]' : 'bg-[hsl(var(--nord-2))]'
            }`}
          >
            ●
          </Button>
          <Button
            ...
            size="sm"
            className={`w-4 h-4 rounded-sm text-xs h-auto p-0 ${
              channel.muted ? 'bg-[hsl(var(--aurora-orange))]' : 'bg-[hsl(var(--nord-2))] hover:bg-[hsl(var(--aurora-orange))]'
            }`}
          >
            M
          </Button>
          <Button
            ...
            size="sm"
            className={`w-4 h-4 rounded-sm text-xs h-auto p-0 ${
              channel.soloed ? 'bg-[hsl(var(--aurora-yellow))]' : 'bg-[hsl(var(--nord-2))] hover:bg-[hsl(var(--aurora-yellow))]'
            }`}
          >
            S
          </Button>
      ...
      <div className="flex items-center space-x-2">
        ...
        <div className="flex flex-col items-center">
          ...
          <span className="text-xs font-mono mt-1">{formatDb(faderPosition)}</span>
        </div>

        {/* Meter */}
        <AudioMeter level={channel.volume} peak={channel.volume + 10} height="h-20" />
      ...
      {channel.name.includes('AI') && (
        <div className="mt-2 space-y-2 text-xs">
          <div className="bg-[hsl(var(--nord-3))] p-2 rounded">
            <div className="text-[hsl(var(--frost-1))] mb-1">Spectral Analysis:</div>
            <div className="h-12 spectral-display rounded mb-1"></div>
            <div className="text-[hsl(var(--nord-4))]">Peak at 2.4kHz detected in vocal range</div>
          </div>
          
          <div className="bg-[hsl(var(--nord-3))] p-2 rounded">
            <div className="text-[hsl(var(--frost-1))] mb-1">AI Recommendations:</div>
            <div className="text-[hsl(var(--nord-4))]">• Apply 3dB cut at 2.4kHz</div>
            <div className="text-[hsl(var(--nord-4))]">• Increase reverb send by 15%</div>
            <div className="text-[hsl(var(--nord-4))]">• Consider parallel compression</div>
          </div>
        </div>
      )}
    </div>
