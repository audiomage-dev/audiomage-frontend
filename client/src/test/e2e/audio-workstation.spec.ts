...
test.describe('Audio Workstation - Critical User Flows', () => {
  ...
  test('should load the main workstation interface @smoke', async ({ page }) => {
  ...
  test('should handle track operations', async ({ page }) => {
    ...
    // Test track mute
    const muteButton = page.locator('[data-testid="track-mute-button"]').first();
    await muteButton.click();
    await expect(muteButton).toHaveAttribute('aria-pressed', 'true');
    
    // Test track solo
    const soloButton = page.locator('[data-testid="track-solo-button"]').first();
  ...
  test('should handle responsive layout', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="compact-controls"]')).toBeVisible();
  });
