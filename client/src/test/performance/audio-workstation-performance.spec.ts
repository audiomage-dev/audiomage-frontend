...
test.describe('Audio Workstation Performance', () => {
  test('should load main interface within performance budget', async ({ page }) => {
  ...
  test('should handle multiple track operations efficiently', async ({ page }) => {
    await page.goto('/studio');
    await page.waitForLoadState('networkidle');
  ...
  test('should handle large audio file processing', async ({ page }) => {
    ...
    if (await fileInput.isVisible()) {
      await fileInput.setInputFiles('attached_assets/file_example_WAV_1MG (1).wav');
    }
    
    // Wait for processing
    await page.waitForSelector('[data-testid="audio-waveform"]', { timeout: 10000 });
